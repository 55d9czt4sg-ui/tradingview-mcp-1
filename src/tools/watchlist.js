...
export function registerWatchlistTools(server) {
  ...
  server.tool(
    ...
    "Add a symbol to the TradingView watchlist",
    {
      symbol: z
        .string()
        .describe("Symbol to add (e.g., AAPL, BTCUSD, ES1!, NYMEX:CL1!)"),
    },
    async ({ symbol }) => {
      try {
  ...
  server.tool(
    ...
    "Remove a symbol from the TradingView watchlist",
    {
      symbol: z
        .string()
        .describe("Symbol to remove (e.g., AAPL, BTCUSD, ES1!, NYMEX:CL1!)"),
    },
    async ({ symbol }) => {
      try {
        return jsonResult(await core.remove({ symbol }));
      } catch (err) {
        // Try to close any open context menu on error
        try {
          const { getClient } = await import("../connection.js");
          const c = await getClient();
          await c.Input.dispatchKeyEvent({
